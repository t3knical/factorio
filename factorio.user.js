...
window.addEventListener('load', function() {
    ws1 = new WebSocket('wss://server.rplace.tk:443' + (localStorage.vip ? "/" + localStorage.vip : ""))
	ws1.onmessage = async function({data}){
...
$("body").append('<div id="mydialog" style="display: none">'
    + '<font style="font-size: 10px"><u>Current Template:</u></font> <font id="currentTemplate" style="font-size: 8px"></font>'
    + '<br /><br />'
    + '<font style="font-size: 10px"><u>Last Placed Pixel (x, y):</u></font> <font id="lastPlacedPixel" style="font-size: 8px"></font>'
    + '<br /><br />'
    + '<font style="font-size: 10px"><u>Pixels Left To Work On:</u></font> <font id="pixelesLeft" style="font-size: 8px"></font>'
    + '<br /><br />'
    + '<font style="font-size: 10px"><u>Current Cooldown:</u></font> <font id="currentCooldown" style="font-size: 8px"></font>'
    + '</div>');
...
$("head").append(
    '<style type="text/css">'
    + '#mydialog { margin: 0; border: 2; height:100%; max-height:100%; color:#000 !important;top: 0.5em; left: 0.5em; outline: 0 none; padding: 0 !important; font-family: Verdana,Arial,sans-serif;vertical-align: baseline; line-height: 0.50; }'
    ...
    + '.ui-dialog-titlebar { padding: 0.5em .5em; position: relative; font-family: Verdana,Arial,sans-serif; font-size: 12px }'
...
var cnc_url0 = 'https://raw.githubusercontent.com/t3knical/rplace/main/Templates/Canada/full_canvas_template_canada_full.png';
var cnc_url1 = 'https://raw.githubusercontent.com/t3knical/rplace/main/Templates/Canada/full_canvas_template_with_allies.png';
var cnc_url2 = 'https://raw.githubusercontent.com/t3knical/rplace/main/Templates/Canada/full_canvas_template_the_blackout.png';
...
function autoput(X, Y, color) {
    if (CD > Date.now() || ws1 == undefined) return
    if(ws1.readyState == 1)
    {
        ...
        ws1.send(a);
		Toastify({
			text: 'Pixel placement command ran till end...',
			duration: 1000,
		}).showToast()
    }    
    return true
...
async function attemptPlace() {
	...
	if (order0 == undefined || order1 == undefined || order2 == undefined) {
        setTimeout(attemptPlace, 1000) // probeer opnieuw in 2sec.
        return
    }
    if (rgbaOrder0 == undefined || rgbaOrder1 == undefined || rgbaOrder2 == undefined) {
    ...
    if (CD <= Date.now()) {
        ...
        if (work0.length >= 50) {
            ...
            document.getElementById('currentTemplate').textContent = `1 - ${percentComplete}% Complete!`;
            ...
            document.getElementById('currentTemplate').textContent = `2 - ${percentComplete}% Complete!`;
            document.getElementById('lastPlacedPixel').textContent = `${x1}, ${y1}`;
            ...
            document.getElementById('currentTemplate').textContent = `3 - ${percentComplete}% Complete!`;
            document.getElementById('lastPlacedPixel').textContent = `${x1}, ${y1}`;
